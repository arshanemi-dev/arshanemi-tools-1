...
const K = {
  ...
  storageProvider: 'lt_storage_provider',
}
...
// ── Storage provider (per-browser choice — no server round trip) ──────────────
// The active provider lives in this browser's localStorage, not in server-side
// config, so switching never touches the filesystem (Vercel's is read-only) and
// never affects other users. It's mirrored into a cookie because localStorage
// never reaches the server — API routes read the cookie to pick the adapter for
// requests, including plain <img> thumbnail requests that can't set headers.

export function getStorageProvider()  { return read(K.storageProvider, null) }

export function setStorageProvider(provider) {
  write(K.storageProvider, provider)
  if (typeof document !== 'undefined') {
    document.cookie = `storage_provider=${provider}; path=/; max-age=31536000; SameSite=Lax`
  }
}
