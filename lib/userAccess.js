...
// Returns the full storage root path for a user (works for whichever provider is active).
// Structure: /tools/<company.folderId>/<user_folder_name>
// Admin users are rooted at /tools/<company.folderId> (or /tools if no company).
export async function buildUserRootPath(user) {
  if (!user) return ''
...
export async function ensureUserFolderFromClient(user) {
  ...
  const rootPath = await buildUserRootPath(user)
