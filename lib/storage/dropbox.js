...
// The SDK's binary-download path (filesGetThumbnailV2, etc.) picks between res.blob()
// and res.buffer() based on isWindowOrWorker() — in a Node server (window undefined,
// module defined) it always takes the res.buffer() branch, expecting a node-fetch-style
// Response. Node's native fetch (undici) has no .buffer(), so that call throws
// "res.buffer is not a function". Patch it on rather than adding a node-fetch dependency.
