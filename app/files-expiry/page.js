...
export default function FilesExpiryPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-auto bg-[var(--lt-bg-base)] p-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-semibold text-[var(--lt-text-primary)]">Files Expiry</h1>
            <p className="text-xs text-[var(--lt-text-subtle)]">Track and manage file expiry dates</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {someChecked && (
            <>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-[var(--lt-accent)]/40 text-[var(--lt-accent-light)] bg-[var(--lt-accent-muted)] rounded-[8px] hover:bg-[var(--lt-accent-muted)]/80 transition-colors"
              >
                <Clock size={12} />
          ...
          <button
            onClick={openAdd}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-[var(--lt-accent)] text-white rounded-[8px] hover:bg-[var(--lt-accent-hover)] transition-colors"
          >
            <Plus size={12} />
            Add Files
          </button>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-48">
          <Loader2 size={20} className="animate-spin text-[var(--lt-accent)]" />
        </div>
      ) : records.length === 0 ? (
        <div className="text-center py-16">
          <AlertTriangle size={32} className="mx-auto mb-3 text-[var(--lt-divider-light)]" />
          <p className="text-sm font-medium text-[var(--lt-text-muted)]">No files tracked yet</p>
          <p className="text-xs text-[var(--lt-text-subtle)]">Click "Add Files" to start tracking expiry dates</p>
        </div>
      ) : (
        <div className="border border-[var(--lt-divider)] rounded-[10px] overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[var(--lt-surface)] border-b border-[var(--lt-divider)]">
              <tr>
                <th className="w-10 px-4 py-3">
                  <input type="checkbox" checked={allChecked} onChange={toggleAll}
                    className="rounded border-[var(--lt-divider-light)] bg-transparent accent-[var(--lt-accent)]" />
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--lt-text-subtle)]">Name</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--lt-text-subtle)]">Expiry Date</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--lt-text-subtle)]">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-[var(--lt-text-subtle)] hidden sm:table-cell">Created</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-[var(--lt-text-subtle)]">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--lt-card-hover)]">
              {records.map(r => {
                const expiryVal = r.expiryAt ?? r.expiry_at
                const days = daysRemaining(expiryVal)
                return (
                  <tr key={r.id} className={selected.has(r.id) ? 'bg-[var(--lt-accent-muted)]/30' : 'hover:bg-[var(--lt-surface)]'}>
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={selected.has(r.id)} onChange={() => toggleOne(r.id)}
                        className="rounded border-[var(--lt-divider-light)] bg-transparent accent-[var(--lt-accent)]" />
                    </td>
                    <td className="px-4 py-3 font-medium text-[var(--lt-text-primary)] truncate max-w-xs">{r.name}</td>
                    <td className="px-4 py-3 text-[var(--lt-text-muted)] text-xs">
                    ...
                    <td className="px-4 py-3 text-[var(--lt-text-subtle)] text-xs hidden sm:table-cell">
                      {new Date(r.createdAt ?? r.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => openEditOne(r)}
                          className="p-1.5 rounded-[6px] hover:bg-[var(--lt-card-hover)] text-[var(--lt-text-subtle)] hover:text-[var(--lt-accent-light)] transition-colors">
                          <Pencil size={13} />
                        </button>
                        <button onClick={() => handleDelete([r.id])}
                          className="p-1.5 rounded-[6px] hover:bg-red-500/10 text-[var(--lt-text-subtle)] hover:text-red-400 transition-colors">
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
        ...
        <Modal title="Add Files to Track" onClose={() => setModal(null)}>
          <div className="space-y-2">
            {addRows.map((row, i) => (
              <div key={i} className="flex gap-2 items-center">
                <input type="text" placeholder="File name or path" value={row.name}
                  ...
                  className="flex-1 bg-[var(--lt-card)] border border-[var(--lt-divider)] rounded-[8px] px-3 py-2 text-sm text-[var(--lt-text-primary)] placeholder:text-[var(--lt-text-subtle)] focus:outline-none focus:ring-1 focus:ring-[var(--lt-accent)]/60"
                />
                <input type="date" value={row.expiryAt}
                  onChange={e => setAddRows(prev => prev.map((r, j) => j === i ? { ...r, expiryAt: e.target.value } : r))}
                  className="bg-[var(--lt-card)] border border-[var(--lt-divider)] rounded-[8px] px-3 py-2 text-sm text-[var(--lt-text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--lt-accent)]/60"
                />
                {addRows.length > 1 && (
                  <button onClick={() => setAddRows(prev => prev.filter((_, j) => j !== i))} className="text-[var(--lt-text-subtle)] hover:text-red-400">
                    <X size={14} />
                  </button>
                )}
              </div>
            ...
            <button onClick={() => setAddRows(prev => [...prev, { ...EMPTY_ROW }])}
              className="text-xs text-[var(--lt-accent-light)] hover:underline">
              + Add another row
            </button>
          </div>
      ...
      {modal === 'editOne' && (
        <Modal title="Edit File Expiry" onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-medium text-[var(--lt-text-muted)] mb-1">File Name</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--lt-text-muted)] mb-1">Expiry Date</label>
      ...
      {modal === 'bulkExpiry' && (
        <Modal title={`Update Expiry for ${selected.size} File(s)`} onClose={() => setModal(null)}>
          <p className="text-xs text-[var(--lt-text-subtle)] mb-3">Set a new expiry date for all selected files.</p>
...
function Modal({ title, onClose, children }) {
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--lt-surface)] border border-[var(--lt-divider)] rounded-[12px] shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--lt-divider)]">
          <h2 className="font-semibold text-[var(--lt-text-primary)] text-sm">{title}</h2>
          <button onClick={onClose} className="text-[var(--lt-text-subtle)] hover:text-[var(--lt-text-primary)]"><X size={16} /></button>
        </div>
  ...
  return (
    <div className="flex justify-end gap-2 mt-4">
      ...
      <button onClick={onSave} disabled={saving}
        className="px-4 py-1.5 text-xs bg-[var(--lt-accent)] text-white rounded-[8px] hover:bg-[var(--lt-accent-hover)] transition-colors disabled:opacity-60 flex items-center gap-1.5">
